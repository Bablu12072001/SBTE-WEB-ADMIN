...
export default function DeletePreviousYearQPModal({
  isOpen,
  setIsOpen,
  item,
  token,
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete("/api/admin/previous_year_question", {
        headers: {
      ...
      if (response.data.status) {
        ...
        alert("Failed to delete the question paper.");
      }
    } catch (error) {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="bg-white p-6 rounded max-w-sm w-full shadow-xl">
          ...
          <p className="text-sm text-gray-700 mb-4">
            Are you sure you want to delete the question paper{" "}
