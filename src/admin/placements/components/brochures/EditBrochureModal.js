...
export default function EditBrochureModal({
  isOpen,
  setIsOpen,
  brochure,
  refreshData,
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      attachment: brochure.attachment ?? "",
      date: brochure.date ? brochure.date.split("T")[0] : "",
    });
  }, [brochure]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.put("/api/admin/placement_broacher", payload, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (res.data && res.data.status) {
        ...
        console.error("PUT /api/admin/placement_broacher returned:", res.data);
        alert("Failed to update brochure. See console for details.");
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
      <div className="bg-white p-6 rounded-lg max-w-md w-full shadow-xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-blue-900">Edit Brochure</h2>
          <button
