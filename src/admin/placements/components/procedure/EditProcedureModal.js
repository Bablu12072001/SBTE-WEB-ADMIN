...
export default function EditProcedureModal({
  ...
  useEffect(() => {
    ...
    setFormData({
      id: procedure.id ?? "",
      txt: procedure.txt ?? "",
      attachment: procedure.attachment ?? "",
    });
  }, [procedure]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.put("/api/admin/procedure", payload, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (res.data && res.data.status) {
        ...
        console.error("PUT /api/admin/procedure returned:", res.data);
        alert("Failed to update procedure. See console for details.");
      }
    } catch (err) {
