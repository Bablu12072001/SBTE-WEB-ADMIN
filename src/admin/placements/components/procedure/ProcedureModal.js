...
export default function ProcedureModal({ isOpen, setIsOpen, onSuccess }) {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (status && path) {
          ...
          alert("Failed to upload file. Unexpected response from server.");
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post("/api/admin/procedure", payload, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      console.error(
        "Error adding procedure:",
        error.response?.data || error.message
      );
      alert("Error submitting procedure. Check console for details.");
    }
  };
