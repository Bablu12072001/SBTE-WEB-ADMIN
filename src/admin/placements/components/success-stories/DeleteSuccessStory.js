...
export default function DeleteSuccessStory({
  open,
  storyId,
  onCancel,
  onSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await axios.delete("/api/admin/stories", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        data: { id: storyId }, // body in delete request
      });
      ...
      if (res.data && res.data.status) {
        ...
        console.error("Unexpected delete response:", res.data);
    ...
    <Dialog open={open} onClose={onCancel} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="bg-white p-6 rounded-lg max-w-sm w-full shadow-xl">
          ...
          <p className="mt-2">
            Are you sure you want to delete this success story (ID: {storyId})?
          </p>
