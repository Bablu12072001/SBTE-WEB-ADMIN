...
export default function EditSuccessStoryModal({
  isOpen,
  setIsOpen,
  story,
  refreshData,
  ...
  useEffect(() => {
    ...
    setFormData({
      id: story.id ?? "",
      name: story.name ?? "",
      collage_name: story.collage_name ?? "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.put("/api/admin/stories", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (res.data && res.data.status) {
        ...
        console.error("PUT /api/admin/stories returned:", res.data);
        alert("Failed to update success story. See console for details.");
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
      <div className="bg-white p-6 rounded-lg max-w-lg w-full shadow-xl">
        ...
        <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div>
            <label className="block text-sm font-medium">LPA</label>
            <input
              type="text"
              name="LPA"
              value={formData.LPA}
